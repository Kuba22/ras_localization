...
class PfHelper{
public:
	static mat readLines(string map_file)
	{
		mat lines(1, 4);
	...
	static vec crossProduct(vec a, vec b) {//maybe use armadillo's cross product //don't use arma::vec but array (saves time converting)
		vec ret(3);
		if (a.size() == 2)
		{
			ret[0] = a[1] - b[1];
	...
	static bool notProperX(rowvec line, vec X, vec P, vec R) {
	...
	static double getRange(mat lines, vec R, double phi) {
		double minR = 100.0;
		double theta = R[2];
		vec P({ 100.0*cos(phi + theta) + R(0), 100.0*sin(phi + theta) + R(1) });//precompute the sin and cos
